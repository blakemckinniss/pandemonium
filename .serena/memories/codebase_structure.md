...
│   ├── seed-content.ts         # Base content seeding (enemies, dungeons)
...
| File | Purpose |
|------|---------|
| `src/types/index.ts` | All TypeScript interfaces (120+ types) |
| `src/game/actions.ts` | `applyAction()` - all combat state mutations |
| `src/game/cards.ts` | Card definitions registry (heroes, enemies, player cards) |
...
| `src/game/dungeon-generator.ts` | AI dungeon deck generation |
| `src/game/seed-content.ts` | Base content seeding |
| `src/content/rooms.ts` | Room/encounter definitions |
| `src/stores/metaStore.ts` | Zustand store for meta-progression |
| `src/stores/db.ts` | Dexie IndexedDB (dungeons, run history) |
