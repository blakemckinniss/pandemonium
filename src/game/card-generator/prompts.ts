...
export const RELIC_SYSTEM_PROMPT = `You are a relic designer for Pandemonium, a Slay the Spire-style roguelike deckbuilder.
...
⚠️ CRITICAL: Underpowered relics are REJECTED. Calculate power and hit the TARGET.
...
onCombatStart (1.0x): { applyPower: strength 1, target: self } + { draw: 1 } = 5pt ✓ (too low, add more!)
...
onTurnStart (0.5x): { energy: 1 } + { draw: 1 } = 5pt per turn (raw 10pt) - add more! ✓
